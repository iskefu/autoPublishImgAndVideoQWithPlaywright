...
async def xhs(browser):
    ...
    await page.locator('div.post button').click()
    print('点击发表视频')
    
    # 选择视频
    print("视频地址:",video)
    # page.once("filechooser", lambda file_chooser: file_chooser.set_files(video))
    await page.locator('span  input[type="file"]').set_input_files(video)
    # await page.get_by_role("button", name="上传视频").click()
    await page.get_by_text("删除").first.wait_for()
    print('上传视频成功')
    
    title= os.getenv('TITLE')
    title=title.replace(',', ' ')
    title=title.replace('，', ' ')
    await page.get_by_placeholder("概括视频主要内容，字数建议6-16个字符").fill(title)
    print('填写标题成功')
